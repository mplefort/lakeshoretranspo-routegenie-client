 ...
 */
async function buildInvoices(inputCsv, outputCsv) {
 ...
 */
async function parseCsvRows(inputCsv) {
    const rows = [];
 ...
 */
function aggregateDetailFields(row, agg, key, payer) {
    for (const [svc, mod, qty, cost] of DETAIL_FIELDS) {
        ...
        for (let i = 0; i < svcCodes.length; i++) {
            ...
            if (!agg[key].items[itemKey])
                agg[key].items[itemKey] = { qty: 0, cost: 0 };
 ...
 */
function aggregateCustomServiceCodes(row, agg, key, payer) {
 ...
 */
function aggregateOrderItems(row, agg, key, payer) {
 ...
 */
function flattenAggregatedResults(agg) {
    const records = [];
    let invoiceNum = 1000;
 ...
 */
async function writeCsvRecords(outputCsv, records) {
