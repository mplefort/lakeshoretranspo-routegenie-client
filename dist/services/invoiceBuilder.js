...
exports.aggregateRows = exports.parseCsvRows = exports.flattenAggregatedResults = exports.buildInvoices = void 0;
const fs_1 = __importDefault(require("fs"));
const fast_csv_1 = require("fast-csv");
const csv_writer_1 = require("csv-writer");
const logger_1 = require("../utils/logger");
const mileageCache_1 = require("./mileageCache");
// --- CONSTANTS & TYPES ---
/**
 ...
 */
function buildInvoices(inputCsv, outputCsv, startingInvoiceNumber = 1000) {
    return __awaiter(this, void 0, void 0, function* () {
        // Initialize the mileage cache
        yield mileageCache_1.mileageCache.initialize();
        logger_1.Logger.info('Mileage cache initialized for invoice building');
        try {
            const rows = yield parseCsvRows(inputCsv);
            const agg = yield aggregateRows(rows);
            const records = flattenAggregatedResults(agg, startingInvoiceNumber);
            yield writeCsvRecords(outputCsv, records);
        }
        finally {
            // Always close the cache connection
            yield mileageCache_1.mileageCache.close();
            logger_1.Logger.info('Mileage cache connection closed');
        }
    });
}
exports.buildInvoices = buildInvoices;
/**
 * Reads and parses CSV rows, skipping the first line.
 */
function parseCsvRows(inputCsv) {
    return __awaiter(this, void 0, void 0, function* () {
        const rows = [];
        yield new Promise((resolve, reject) => {
            const readStream = fs_1.default.createReadStream(inputCsv);
...
function aggregateRows(rows) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const row of rows) {
            ...
            if (!isNaN(Number(clientAuth)) && Number(clientAuth) > 1e15)
                clientAuth = '';
            ...
            const cacheEntry = yield getCachedMileageData(row);
            yield aggregateDetailFields(row, agg, key, payer, orderId, cacheEntry);
            yield aggregateCustomServiceCodes(row, agg, key, payer, orderId, cacheEntry);
 ...
 */
function aggregateDetailFields(row, agg, key, payer, orderId, cacheEntry) {
    return __awaiter(this, void 0, void 0, function* () {
        for (const [svc, mod, qty, cost] of DETAIL_FIELDS) {
            const svcCode = row[svc];
            if (!svcCode)
                continue;
            const svcCodes = String(svcCode).split(',').map((s) => s.trim()).filter(Boolean);
            const modCodes = String(row[mod] || '').split(',').map((s) => s.trim());
            const qtyVals = String(row[qty] || '').split(',').map((s) => s.trim());
            const costVals = String(row[cost] || '').split(',').map((s) => s.trim());
            for (let i = 0; i < svcCodes.length; i++) {
                ...
                if (code === 'S0215' && qty === 'Order Mileage Quantity' && cacheEntry) {
                    const cachedMiles = mileageCache_1.mileageCache.getCachedMileage(cacheEntry);
                    quantity = cachedMiles;
                    logger_1.Logger.info(`Using cached mileage for ${key}: ${cachedMiles} miles (RG: ${cacheEntry.RG_miles}, Google: ${cacheEntry.Google_miles})`);
                }
                ...
                if (!agg[key].items[itemKey])
                    agg[key].items[itemKey] = { qty: 0, cost: 0, orderIds: new Set() };
                agg[key].items[itemKey].qty += quantity;
                agg[key].items[itemKey].cost += costVal;
                if (orderId)
                    agg[key].items[itemKey].orderIds.add(orderId);
            }
        }
    });
}
 ...
 */
function aggregateCustomServiceCodes(row, agg, key, payer, orderId, cacheEntry) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (customField && typeof customField === 'string') {
            ...
            while ((match = CUSTOM_SERVICE_CODE_REGEX.exec(customField)) !== null) {
                ...
                if (code === 'S0215' && cacheEntry) {
                    const cachedDeadMiles = mileageCache_1.mileageCache.getCachedDeadMileage(cacheEntry);
                    quantity = cachedDeadMiles;
                    logger_1.Logger.info(`Using cached dead mileage for ${key}: ${cachedDeadMiles} miles (RG: ${cacheEntry.RG_dead_miles}, Google: ${cacheEntry.Google_dead_miles})`);
                }
 ...
 */
function aggregateOrderItems(row, agg, key, payer, orderId) {
 ...
 */
function flattenAggregatedResults(agg, startingInvoiceNumber = 1000) {
    const records = [];
    for (const passengerAuth of Object.keys(agg)) {
        ...
        for (const item of Object.keys(agg[passengerAuth].items)) {
            ...
            records.push({
                ...
                OrderIds: Array.from(orderIds).sort(),
...
exports.flattenAggregatedResults = flattenAggregatedResults;
/**
 * Writes the final records to a CSV file.
 */
function writeCsvRecords(outputCsv, records) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield writer.writeRecords(recordsWithNumbers);
        logger_1.Logger.success(`Invoices written to ${outputCsv}`);
    });
}
/**
 ...
 */
function parseOrderItems(orderItems, serviceCodes, modifiers, payer) {
 ...
 */
function getCachedMileageData(row) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!firstName || !lastName || !puAddress || !doAddress) {
            logger_1.Logger.warn(`Missing required address data for ${firstName} ${lastName}, skipping cache lookup`);
            return null;
        }
        const cacheParams = {
            firstName,
        ...
        try {
            ...
            let cacheEntry = yield mileageCache_1.mileageCache.findCacheEntry(cacheParams);
            if (!cacheEntry) {
                // Create new cache entry with Google Maps API calls
                logger_1.Logger.info(`Creating new cache entry for ${firstName} ${lastName}`);
                cacheEntry = yield mileageCache_1.mileageCache.createCacheEntry(cacheParams, rgMiles, rgDeadMiles);
            }
            return cacheEntry;
        }
        catch (error) {
            logger_1.Logger.error(`Error with mileage cache for ${firstName} ${lastName}:`, error);
            return null;
        }
    });
}
//# sourceMappingURL=invoiceBuilder.js.map